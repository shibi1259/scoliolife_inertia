...
function selectallcategory() {
  $(".menu-list input").each(function (key, item) {
    $(item).click();
  });
...
function selectallpost() {
  $(".post-list input").each(function (key, item) {
    $(item).click();
  });
  // if($('#select-all').is(':checked')) {
...
function addcategorymenu() {
    ...
    var names = [];
    var slugs = [];
    $(".menu-list input[type='checkbox']").each(function (key, item) {
        if ($(item).is(":checked")) {
            let id = $(item).val();
          ids.push(id);
          let name = $(`.menu-list input[name='name-${id}']`).val();
          names.push(name);

          let slug = $(`.menu-list input[name='slug-${id}']`).val();
          slugs.push(slug);
        }
    });
...
function addpostmenu() {
    $("#spincustomu").show();
    var ids = [];
    var names = [];
    var slugs = [];

    $("li.post-list input[type='checkbox']").each(function (key, item) {
        if ($(item).is(":checked")) {
            let id = $(item).val();
          ids.push(id);
          let name = $(`.post-list input[name='name-${id}']`).val();
          names.push(name);

          let slug = $(`.post-list input[name='slug-${id}']`).val();
          slugs.push(slug);
        }
    });
